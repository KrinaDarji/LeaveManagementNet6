...
namespace LeaveManagement.Web.Repositories
{
    public class LeaveRequestRepository : GenericRepository<LeaveRequest> , ILeaveRequestRepository
    {
        ...
        public LeaveRequestRepository(ApplicationDbContext context,IMapper mapper, IHttpContextAccessor httpContextAccessor,ILeaveAllocationRepository leaveAllocationRepository, UserManager<Employee> userManager,IEmailSender emailSender) : base(context)
        {
        ...
        public async Task CancelLeaveRequest(int leaverequestId)
        {
            ...
            await emailSender.SendEmailAsync(user.Email, $"Leave Request Cancel", $"Your leave request from" + $"{leaveRequest.StartDate} to {leaveRequest.EndDate} has been Cancelled");
        ...
        public async Task<bool> CreateLeaveRequest(LeaveRequestCreateVM model)
        {
            ...
            await emailSender.SendEmailAsync(user.Email,"Leave Request submiited Successfully",$"Your leave request from"+$"{leaveRequest.StartDate} to {leaveRequest.EndDate} has been submitted for approval");
            return true;
        }

        public async Task<AdminLeaveRequestVM> GetAdminLeaveRequestViewVM()
        {
            var leaveRequests = await context.LeaveRequests.Include(q=>q.LeaveType).ToListAsync();
            var model = new AdminLeaveRequestVM
            {
                TotalRequests = leaveRequests.Count,
                Approved = leaveRequests.Count(q => q.Approved == true),
                Pending = leaveRequests.Count(q => q.Approved == null),
                Rejected = leaveRequests.Count(q => q.Approved == false),
